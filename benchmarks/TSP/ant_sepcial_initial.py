...
num = 5
dataframe = pd.read_csv("./data/TSP"+str(num)+"users.tsp",sep=" ",header=None)
...
def getdistmat():
    distmat = np.zeros((coordinates.shape[0]+1,coordinates.shape[0]+1))
    for i in range(train_v.shape[0]):
        for j in range(train_d.shape[0]):
            distmat[i,j] = math.sqrt(np.sum((train_v[i,:]-train_d[j,:])**2))
        if i == train_v.shape[0] - 1:
            distmat[i, j + 1] = 0.0001
        else:
            distmat[i, j + 1] = 100000
    for index in range(train_v.shape[0] + 1):
        if index == train_v.shape[0] - 1 or train_v.shape[0]:
            distmat[train_v.shape[0], index] = 0.0001
        else:
            distmat[train_v.shape[0], index] = 100000
    return distmat

distmat = getdistmat()

numant = 10 #蚂蚁个数
numcity = coordinates.shape[0]+1 #城市个数
alpha = 1   #信息素重要程度因子
beta = 5    #启发函数重要程度因子
rho = 0.1   #信息素的挥发速度
Q = 1
...
index = 0
#作出找到的最优路径图
bestpath = list(pathbest[-1])
print(bestpath)
bestpath_special = np.zeros(numcity-1)
for i in range(numcity):
    if bestpath[i]==num+1:
        index = i
print(index)
if index == numcity-1:
    bestpath_special = bestpath[:index]
else:
    bestpath_special = bestpath[index+1:]+bestpath[:index]
print(bestpath_special)

plt.plot(coordinates[:,0],coordinates[:,1],'r.',marker=u'$\cdot$')
#plt.xlim([-100,3500])
#plt.ylim([-100,3500])
dis = 0.0
for i in range(numcity-1-1):#
    m,n = int(bestpath_special[i]),int(bestpath_special[i+1])
    print(np.sqrt((coordinates[m][0] - coordinates[n][0])**2 + (coordinates[m][1] - coordinates[n][1])**2))
    dis += np.sqrt((coordinates[m][0] - coordinates[n][0])**2 + (coordinates[m][1] - coordinates[n][1])**2)
    print (m,n)
...
Complete_time = dis * 100 / 20 + 22.5 * num
Energy = (dis * 100 / 20) * 178.2958 + 22.5 * num * 168.4842
